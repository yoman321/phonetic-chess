...
def make_sessions_bp(db, socketio):
    ...
    def create_session():
        ...
        return jsonify(sessions_ops.create_session(db, data.get("color"))), 201

    @bp.get("/sessions/<sid>")
    def get_session(sid):
        return jsonify(sessions_ops.get_session(db, sid))

    @bp.post("/sessions/<sid>/join")
    def join_session_route(sid):
        data = request.get_json(silent=True) or {}
        return jsonify(
            sessions_ops.join_session(db, socketio, sid, data.get("playerToken"))
        )
    ...
    def make_move(sid):
        ...
        return jsonify(sessions_ops.make_move(db, socketio, sid, uci, token))
