...
from controller_operations.llm import CallLog, explain_move_with_llm, pick_move_with_llm
...
def say_move(db, socketio, sid, text, token):
    ...
    with db() as pg, pg.transaction(), pg.cursor() as cur:
        ...
        try:
            ...
            moves_q.insert_move(
                ...
                player_text=text, pre_move_fen=row["fen"], prior_tone=prior_tone,
            )
        finally:
    ...
    # Only a committed move is eligible for best-effort analysis logging.
...
def explain_move(db, sid, ply, player_token):
    # Analysis writes are isolated from authentication, generation and caching.
    try:
        with db() as pg, pg.transaction(), pg.cursor() as cur:
            llm_calls_q.increment_explain_requests(cur, sid, ply)
    except Exception:
        logger.exception("explain_move: could_not_count_request sid=%s", sid)

    if not player_token:
        raise ApiError("missing_token", 401)
    with db() as pg, pg.cursor() as cur:
        session = sessions_q.select_tokens(cur, sid)
        if not session:
            raise ApiError("not_found", 404)
        if player_token not in (session["white_token"], session["black_token"]):
            raise ApiError("not_a_player", 403)
        move = moves_q.get_for_explain(cur, sid, ply)
    if not move:
        raise ApiError("not_found", 404)
    if move["intent"] is not None and move["rationale"] is not None:
        return {"intent": move["intent"], "rationale": move["rationale"]}
    if move["player_text"] is None or move["pre_move_fen"] is None:
        raise ApiError("explanation_unavailable", 409)

    started_at = time.monotonic()
    try:
        intent, rationale, usage = explain_move_with_llm(
            move["player_text"], move["pre_move_fen"], move["prior_tone"],
            move["uci"], move["san"],
        )
    except (urllib.error.URLError, TimeoutError) as e:
        raise ApiError("llm_unavailable", 502, detail=str(e)) from e
    except (ValueError, json.JSONDecodeError, KeyError) as e:
        raise ApiError("llm_bad_response", 502, detail=str(e)) from e
    latency_ms = int((time.monotonic() - started_at) * 1000)

    with db() as pg, pg.transaction(), pg.cursor() as cur:
        moves_q.save_explanation(cur, sid, ply, intent, rationale)
    try:
        with db() as pg, pg.transaction(), pg.cursor() as cur:
            llm_calls_q.save_explanation(
                cur, sid, ply, intent, rationale, usage, latency_ms
            )
    except Exception:
        logger.exception("explain_move: could_not_persist_analysis sid=%s", sid)
